...
function Question({
  ...
  return (
    <article onClick={() => console.log(questionIndex)} className={`${styles.container} ${isActive ? styles.hit : ''}`}>
                    ...
                    <Draggable draggableId={option.id} key={option.id} index={idx}>
                      {(provided) => (
                        ...
                        >
                          ...
                          ></input>
                          <div
                            className={`${styles.targetSelectArea} ${optionTargetOpenIdx === idx ? styles.open : ''}`}
                            onClick={() => setOptionTargetOpenIdx((prev) => (prev === idx ? -1 : idx))}
                            tabIndex={1}
                          ...
                          >
                            <span>
                              {(() => {
                                ...
                                if (target.length === 0) return '해당 옵션 선택시 활성화';
                                else return target.map((question) => question.question).join(', ');
                              })()}
                            </span>
                            {optionTargetOpenIdx === idx && (
                              <ul className={styles.targetList}>
                                {(() => {
                                  ...
                                  return list.map((q) => (
                                    <li
                                      className={option.target.includes(q.id) ? styles.hit : ''}
                                      key={question.id}
                                      onClick={() => updateOptionTarget(idx, q.id)}
                                    >
                                      {q.question}
                                    </li>
                                  ));
                          ...
                          <button className={styles.removeOptionBtn} onClick={() => removeOption(option.id)}>
